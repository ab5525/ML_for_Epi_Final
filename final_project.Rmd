...
pacman::p_load(pacman, Amelia, caret, cluster, devtools,e1071, factoextra, gbm, GGally, ggplot2, ggthemes, 
  ggvis, ggbiplot, glmnet,httr, knitr,lubridate, plotly, pROC,randomForest, rio, rpart, rpart.plot, rmarkdown, shiny, stringr,SuperLearner, stats,tidyr, tidyverse) 
...
## Data Cleaning
Omit missings, mutate the DF to combine some variables for simplicity
...
head(df)
```
This leaves us with 4158 subjects in total. We have 47 columns in total.



## Create a propensity score

We will create a score for exposure based on race, gender and age. 
...
df[df$interest_cluster == 1, c(interest_cols,children_cols)]
```

### Split the data and remove unneeded columns

Variable explanation: place_1 refers to someone having gone to the location during the pandemic at the start of the study. place_2 refers to them having gone to the location 2 years later. We will consider these as exposure risks.
```{r split the data}
df <-subset(df, select=-booster)
df <-subset(df, select=-vax_comb)
...
This model uses logistic regression to determine.
...
set.seed(123)
logit<-train(vaccinated~ PS + hoax + early_vax_interest +antiviral + employed + combined_any, data=fdf.train, method="glm", family=fam, preProcess=c("center", "scale"), trControl=control.obj)
...
gbm.grid <- data.frame(.n.trees=(1:4)*50, .shrinkage=0.6, .interaction.depth=c(2,4), .n.minobsinnode=5)

gbm.model <- train(vaccinated~ age + race + gender + hoax + antiviral + early_vax_interest + combined_1, data=fdf.train,
                   distribution="bernoulli", method="gbm",
                   trControl=trainControl, verbose=FALSE, tuneGrid=gbm.grid, metric='Accuracy',) 
gbm.model$results
...
set.seed(123)
    rf.model<-train(vaccinated~ PS + hoax + antiviral + early_vax_interest + combined_1, data=fdf.train, method="rf", trControl=control.obj2, metric="Accuracy", tuneGrid=mtry.grid2, importance=TRUE,ntree=as.numeric(best.tune2$ntrees))
...
```{r test_set}
set.seed(123)
print('logisitic regression predictions in the test set')
logit.prob<- logit %>% predict(fdf.test)
logit.preds<- predict(logit, fdf.test, type="prob")
logit.probability<-logit.preds[,2]
logit.cm = confusionMatrix(logit.prob, fdf.test$vaccinated, positive = 'TRUE')
logit.cm


print('Random Forest Predictions in the test set')

rf.prob<- rf.model %>% predict(fdf.test)
rf.preds2<- predict(rf.model2, fdf.test, type="prob")
rf.probability<-rf.preds2[,2]
rf.cm = confusionMatrix(rf.prob, fdf.test$vaccinated, positive = 'TRUE')
rf.cm

print('GBM predictions in the test set')

gbm.prob<- gbm.model %>% predict(fdf.test)
gbm.preds<- predict(gbm.model, fdf.test, type="prob")
gbm.probability<-gbm.preds[,2]
gbm.cm = confusionMatrix(gbm.prob, fdf.test$vaccinated, positive = 'TRUE')
gbm.cm
...
#Logistic regression
set.seed(123)
cal.data.index<-fdf.test$vaccinated%>% createDataPartition(p=0.5, list=F)
cal.data<-fdf.test[cal.data.index, ]
final.test.data<-fdf.test[-cal.data.index, ]

#Predict on test-set without scaling to obtain raw pred prob in test set
lm.probs.nocal<-predict(logit, final.test.data, type="prob")
lm.pp.nocal<-lm.probs.nocal[,2]

#Apply model developed on training data to calibration dataset to obtain predictions
lm.probs.cal<-predict(logit, cal.data, type="prob")
lm.pp.cal<-lm.probs.cal[,2]

#Add to dataset with actual values from calibration data
calibrf.data.frame<-data.frame(lm.pp.cal, cal.data$vaccinated)
...
data.test.lm<-data.frame(lm.pp.nocal)
colnames(data.test.lm)<-c("x")
platt.data.lm<-predict(calibrf.model, data.test.lm, type="response")
...
#Add to dataset with actual values from calibration data
calibrf.data.frame<-data.frame(rf.pp.cal, cal.data$vaccinated)
colnames(calibrf.data.frame)<-c("x", "y")
...
gbm.nocal<-predict(gbm.model,final.test.data, type="prob")
gbm.pp.nocal<-gbm.nocal[,2]


#Apply model developed on training data to calibration dataset to obtain predictions
gbm.cal<-predict(gbm.model,cal.data, type="prob")
gbm.pp.cal<-gbm.cal[,2]

#Add to dataset with actual values from calibration data

calib.data.frame<-data.frame(gbm.pp.cal, cal.data$vaccinated)
...
data.test<-data.frame(gbm.pp.nocal)
colnames(data.test)<-c("x")
platt.data<-predict(calib.model, data.test, type="response")

platt.prob.lm<-data.frame(Class=final.test.data$vaccinated, lm.platt=platt.data.lm, lm=lm.pp.nocal)

platt.prob<-data.frame(Class=final.test.data$vaccinated, gbm.platt=platt.data, gbm=gbm.pp.nocal)

platt.prob.rf<-data.frame(Class=final.test.data$vaccinated, rf.platt=platt.data.rf, rf=rf.pp.nocal)


#Plot logistic regression calibration curve
calplot.rf<-(calibration(Class ~ lm.platt+lm, data=platt.prob.lm, class=TRUE, cuts=10))
...
calplot<-(calibration(Class ~ gbm.platt+gbm, data=platt.prob, class=TRUE, cuts=10))
...
### Predictions about if a person had covid or not 
